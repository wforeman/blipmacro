  ...
  /*0*/   //{ "BlipAna_20230108_BiPo_OverlayNEST_scale120_8ms.root",    "blipanaTrkMask/anatree", true,   0,0},
  /*0*/  //{ "BlipAna_20230108_BiPo_Overlay_8ms.root",         "blipanaTrkMask/anatree", true,   0,0},
  ...
  float fAlphaEnergy_max  = 0.24; //0.24; //0.24; 
  ...
  */
  ...
  void makeHistograms()
  {
    ...
    h_clust_mult      = new TH1D("clust_mult","Cluster multiplicity per wire per evd",30,0,30);
  ...
  void BiPo_macro()
  {
    ...
    for(auto iwire : fNoisyWires )  wireIsNoisy[iwire] = true;
        ...
        clust_isAlpha.assign(kMaxClusts, false); 
        clust_isBeta.assign(kMaxClusts, false); 
        clust_isGamma.assign(kMaxClusts, false); 
    ...
    for(size_t iEvent=0; iEvent < fTree->GetEntries(); iEvent++){
      ...
      if(  (iEvent % 1000 /*sparsify*/) != 0 ) continue; 
      //..................................
      ...
      std::vector<int> collection_clusts;
      for(int i=0; i < nclusts; i++){
        ...
        if( clust_deadwiresep[i] == 0 ) _clustAvailable[i]=false;
        if( clust_plane[i] != 2 ) continue;
        collection_clusts.push_back(i);
      ...
      /*
      // Mark 'unavailable' any collection clusters that aren't well-isolated
      // skip if this cluster is at the very edge of the wireplane
      //for(size_t i=0; i<nclusts; i++){
      for(auto& i : collection_clusts ) {
        //if( clust_plane[i] != 2 ) continue;
        if( !_clustAvailable[i] ) continue;
        int w_start = std::max(0,clust_startwire[i]-5);
        int w_end   = std::min(clust_endwire[i]+5,(int)nWiresColl-1);
        if( w_start < 20 || w_end > int(nWiresColl-20) ) {
          ...
          for(int iwire = w_start; iwire <= w_end; iwire++){
            for(auto& jc : _map_wire_clusters[iwire] ) {
              if( i == jc ) continue;
              float dt = fSamplePeriod*fabs(clust_time[jc]-clust_time[i]);
              if( dt < 20 ) {
                _clustAvailable[i] = false;
      ...
      */
      ...
      for(int iBlip=0; iBlip<nblips; iBlip++){
        ...
        for(auto& thisCand : v_cands) {
         ...
         // if( fabs(eventHr-31)<1 && thisCand.dT < 60 ){
         // std::cout<<"Evt "<<event<<", run "<<run<<", hour "<<eventHr<<", beta clust/wire: "<<ic<<"   "<<clust_startwire[ic]<<" - "<<clust_endwire[ic]
         ...
         //   std::cout<<"charge: "<<clust_charge[ic]<<"        "<<clust_charge[thisCand.id2]<<"\n";
        ...
        // ---------------------------------------------
        std::vector<BiPoCandidate> v_control      = FindCandidates(ic, 10, 5, false, nclusts_inwindow, nwires);
        std::vector<BiPoCandidate> v_control_bg = FindCandidates(ic, 10, 5, true,  nclusts_inwindow, nwires);
    ...
    std::cout<<"Backward correction at 10: "<<f_backward_corr->Eval(10)<<"\n";
  ...
  std::vector<BiPoCandidate> FindCandidates(int ic, int wire_shift, int range, bool flipdT, int& npileup, int& nwires ) {
    ...
    for(auto iWire : wires ) {
      ...
      for(auto& jc : _map_wire_clusters[iWire] ) {
        ...
        float beta_E =  blip_energy[blipID];
        float alpha_q = clust_charge[jc]*blip_yzcorr[blipID];
        float alpha_E = (23.6e-6)*(alpha_q/fRecomb);
